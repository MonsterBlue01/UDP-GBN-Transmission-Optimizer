...
Definition: 1.3.1:  Experiment and Event. An experiment is any process, real or hypothetical, in which
            ...
            1.5.1:  Probability. A probability measure, or simply a probability, on a sample space S is a
...
Theorem:    1.4.1:  Let A, B, and C be events. Then A ⊂ S. If A ⊂ B and B ⊂ A, then A = B. If A ⊂ B
            ...
            1.5.3:  For every event A, Pr(A^c) = 1 − Pr(A).

            1.5.4:  If A ⊂ B, then Pr(A) ≤ Pr(B).

            1.5.5:  For every event A, 0 ≤ Pr(A) ≤ 1.

            1.5.6:  See the book.

            1.5.7:  For every two events A and B, Pr(A ∪ B) = Pr(A) + Pr(B) − Pr(A ∩ B).
